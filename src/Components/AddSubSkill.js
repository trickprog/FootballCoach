...
export default function AddSubSkill(props) {
  ...
  return (
    <>
      <div>
        {opensubcat === false ? (
          <div
            ...
            class="p-6 max-w-sm max-h-[180px] rounded-lg bg-no-repeat"
            style={{ backgroundImage: `url("${Bg}")` }}
          >
            <div className="flex justify-center my-5">
              <img src={require(`../assets/${props.img}.png`)} />
            </div>
            <p class="mb-3 text-center text-xl font-semibold text-white">
              {props.name}
            </p>
          </div>
        ) : (
      ...
      <div
        id="defaultModal"
        class={!openAddsubcatmodal?"hidden":" flex absolute top-0 right-0 left-0 z-50 w-full h-full  bg-black bg-opacity-5 justify-center items-center"}
      >
